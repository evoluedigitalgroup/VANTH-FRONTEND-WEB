...
const PartnerDocument = ({ obj, handleShowImageModal }) => {
	return (
		<Col
			md={4}
			style={{
			...
			<Col>
				{obj?.partnerDocument === null && !obj?.docStatus?.partnerDocument && (
					<Button
						className='w-100 p-0 ms-0'
						onClick={
							obj?.partnerDocument === null
								? null
								: () =>
				...
				{obj?.partnerDocument && !obj?.partnerDocument?.approved && (
					<Button
						className='w-100 p-0 ms-0'
						onClick={
							obj?.partnerDocument === null
				...
				{obj?.partnerDocument === null && obj?.docStatus?.partnerDocument && (
					<Button
						...
						onClick={
							obj?.partnerDocument === null
								? null
								: () => handleShowImageModal(obj, "partnerDocument")
						}
